...
const promises = [
	{
		icon: (
			<Binoculars className="w-10 h-10 text-orange-400 group-hover:text-neon-green transition-colors" />
		),
	...
	{
		icon: (
			<ChartColumnIncreasing className="w-10 h-10 text-purple-400 group-hover:text-neon-green transition-colors" />
		),
		title: "ROI Focused",
	...
	{
		icon: (
			<ShieldCheck className="w-10 h-10 text-emerald-400 group-hover:text-neon-green transition-colors" />
		),
	...
	{
		icon: (
			<Rocket className="w-10 h-10 text-blue-400 group-hover:text-neon-green transition-colors" />
		),
			...
			<div className="max-w-7xl mx-auto text-center space-y-16">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
					{promises.map((item, index) => (
						...
						>
							<div
								className={`w-20 h-20 mx-auto rounded-full flex items-center justify-center mb-6 ${item.bg} group-hover:scale-110 transition-transform duration-300`}
							>
								{item.icon}
							</div>
